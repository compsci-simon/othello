...
void read_processes(FILE* fptr, char *line, int type);
 ...
 */
void parse_process_file(char* filename, int type) {
  FILE *fptr = NULL;
  char line[1024];
  int status;
 ...
 * @param type The type of scheduling being used. This deteremines whether
 * a file has priorities for a process.
 */
void read_processes(FILE* fptr, char* line, int type) {
  char *processName;
  char *priority;
  ...
  if (strcmp(line, PROCESSES)==0) {
    ...
    priority = malloc(3*sizeof(char));
    if (type == 1) {
      while(read_string(fptr, processName) != 0 && read_string(fptr, priority)) {
        
        load_process(processName, ((int) *priority) - 48);
        processName = malloc(3*sizeof(char));
        priority = malloc(3*sizeof(char));
  ...
  if (strcmp(line, PROCESS) == 0) {
    ...
    while((s = read_string(fptr, resource_name)) != 0 && s != 2){
      if (strcmp(resource_name, REQ) == 0) {
        ...
        load_instruction(process_name, REQ, resource_name, NULL);
