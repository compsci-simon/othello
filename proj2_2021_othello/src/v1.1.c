...
void gen_move(char *move){
	...
	copy_array(board, root.node_board, BOARDSIZE);
	int depth = DEPTH;
graph_size = 0;
...
graph_size += moves[0];
	if (depth > 0) {
		for (int i = 1; i < moves[0] + 1; i++) {
			copy_array(current->node_board, board, BOARDSIZE);
			struct Node *new_node = malloc(sizeof(struct Node));
			current->children[i-1] = new_node;
			new_node->parent = current;
			new_node->move = moves[i];
			makemove(moves[i], level_colour);
			copy_array(board, new_node->node_board, BOARDSIZE);
			my_strategy(new_node, depth-1, opponent(level_colour));
		}
	}
...
//	fp = fopen("output.txt", "a");
	...
	free(current->children);
		if (level_colour == my_colour) {
			...
			for (int i = 1; i < moves[0] + 1; i++) {
				int score = potential_move_score(moves[i], level_colour);
		...
		if (level_colour == my_colour) {
			int max = -1000000;
			for (int i = 1; i < moves[0] + 1; i++) {
				if (current->children[i-1]->score > max) {
					max = current->children[i-1]->score;
					current->score = max;
					current->move = moves[i];
				}
				free(current->children[i-1]);
			}
			free(current->children);
			return 1;
		} else {
			int min = 1000000;
			for (int i = 1; i < moves[0] + 1; i++) {
				if (current->children[i-1]->score < min) {
					min = current->children[i-1]->score;
					current->score = min;
					current->move = moves[i];
				}
				free(current->children[i-1]);
